...
## 2. 3306 포트가 비어 있는지 확인

로컬에 MySQL을 따로 설치해서 쓰고 있었다면 3306 포트가 이미 점유되어 있어서 `mysql` 컨테이너가 뜨지 못합니다. 아래 에러가 그 증상입니다.
...
가장 오른쪽 열이 PID입니다. 작업 관리자에서 해당 PID(보통 `mysqld`)를 찾아 종료하거나, 로컬 MySQL 서비스 자체를 중지하세요.
...
lsof -i :3306
# 필요 시
brew services stop mysql   # macOS, Homebrew로 설치한 경우
...
| `mysql` 컨테이너가 포트 바인딩 실패로 못 뜸 | 로컬 3306 포트 점유 → 2번 항목 참고 |
